...
private:
    ...
    size_t numNodos; // Tamano del arbol
...
{
    assert(vacio());
    r = new celda{e}; // padre, hizq, hder == nullptr == NODO_NULO
    numNodos = 1;
}
...
inline void Abin<T>::eliminarHijoIzqdo(nodo n)
{
    ...
    assert(n->hizq->hizq == NODO_NULO &&
           n->hizq->hder == NODO_NULO); // y es hoja.
    delete n->hizq;
    n->hizq = NODO_NULO;
    --numNodos;
}
...
inline void Abin<T>::eliminarHijoDrcho(nodo n)
{
    ...
    assert(n->hder->hizq == NODO_NULO &&
           n->hder->hder == NODO_NULO); // y es hoja.
    delete n->hder;
    n->hder = NODO_NULO;
    --numNodos;
}
...
inline Abin<T>::Abin(const Abin& A)
    : Abin{} // Garantiza destruccion de copia incompleta de A.
{
    r = copiar(A.r); // Copiar raiz y descendientes.
    numNodos = A.numNodos;
}
...
typename Abin<T>::nodo Abin<T>::copiar(nodo n)
{
    ...
    if (n != NODO_NULO) {
        ...
        A.r = new celda{n->elto}; // Copiar n en raiz.
        A.r->hizq = copiar(n->hizq); // Copiar subarbol izqdo.
        if (A.r->hizq != NODO_NULO) {
            A.r->hizq->padre = A.r;
        }
        A.r->hder = copiar(n->hder); // Copiar subarbol drcho.
