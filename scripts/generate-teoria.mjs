...
const pdfPath = join(projectRoot, 'static', 'ednl', 'TeoriaEDNL.pdf');
const outDir = join(projectRoot, 'docs', 'teoria');
...
async function outlineToFlat(pdf) {
  ...
  async function walk(items, depth) {
    for (const it of items) {
      ...
      const pageIndex = await resolvePageIndex(it.dest);
      out.push({title, depth, pageIndex});
...
async function main() {
  ...
  if (sections.length === 0) throw new Error('No outline sections found for TeoriaEDNL.pdf');
  ...
  const index =
    frontmatterTitle('Teoría (completa)') +
    "import useBaseUrl from '@docusaurus/useBaseUrl';\n\n" +
    ...
    `## Índice\n\n` +
    links.map(l => `- [${l.title}](/teoria/generado/${l.slug})`).join('\n') +
    '\n';

  writeFileEnsured(join(outDir, 'index.mdx'), index);
  console.log(`OK: wrote teoría docs into ${outDir}`);
}
